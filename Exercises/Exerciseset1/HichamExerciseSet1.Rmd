...
**Your Answers here:**
<br>https://github.com/LucidFusionLabs/market
<br>https://github.com/Skalman/owl_analytics
...
3. *(Extra points)* What if we want to also see the returns of another company, say Amazon!, in the same period? Can you get that data and report the statistics for Amazon!'s stock, too? 

**Your Answers here:**
<br>Question 1:
<br>The code downloads the returns of Apple in the dataset1 using the code:
<br>website_used = "yahoo" # can be "yahoo" or other ( see help(getSymbols) ). Depending on the website we may need to <br>change the stock tickers' representation
<br>mytickers = c("SPY", "AAPL", "IBM", "YHOO")  # Other tickers for example are "GOOG", "GS", "TSLA", "FB", "MSFT", 
<br>startDate = "1996-01-01"
<br># "\nDownloading ticker  AAPL  ..."

<br>Question 2:
<br>The cumulative returns of Apple during this period is r round(100*sum(StockReturns[,2]),1)%. The average daily returns of Apple during this period is r round(100*mean(StockReturns[,2]),3)%; The standard deviation of the daily returns of Apple during this period is r round(100*sd(StockReturns[,2]),3)%;
```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE,results='asis',fig.align='center', fig.height=4,fig.width= 6, fig=TRUE}
APPL = StockReturns[,"AAPL"]
pnl_plot(APPL)
```
...
**Your Answers here:**
<br> Question 1:
<br>Rows = nrow(t(StockReturns)) 
<br>ncol = (t(StockReturns))

<br>Question 2:
<br><br>portfolio = apply(StockReturns,1,mean)
...
**Your Answers here:**
<br> Question 1:
<br> We need to change the following chunk of code  :
<br> " website_used = "yahoo" # can be "yahoo" or other ( see help(getSymbols) ). Depending on the website we may need to <br>change the stock tickers' representation
<br>mytickers = c("SPY", "AAPL", "IBM", "YHOO" ,"AMZN")  # Other tickers for example are "GOOG", "GS", "TSLA", "FB", <br>"MSFT",startDate = "1996-01-01"
<br>
<br>
<br>We need to change this --> startDate="2000-12-29"
<br>
<br> Question 2:
<br>we can add more companies in the DataSet1.R file and using the mean function when we create the portfolio
<br>
<br>
<br>
<hr>
...
Do not get surpsised if you see the csv file in your directories suddenly! You can then read the data from the csv file using the read.csv command. For example, this willsu load the data from the csv file and save it in a new variable that now is called "myData": 
...
**Your Answers here:**
<br> Question 1:
<br> sum(myData != StockReturns[1:20,])
<br>[1] 20
<br> 
<br>20 is the number of days
<br>
